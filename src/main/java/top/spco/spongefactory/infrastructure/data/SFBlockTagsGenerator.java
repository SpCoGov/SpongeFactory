...
public class SFBlockTagsGenerator extends BlockTagsProvider {
    public SFBlockTagsGenerator(DataGenerator dataGenerator, @Nullable ExistingFileHelper existingFileHelper) {
        super(dataGenerator, SpongeFactory.MOD_ID, existingFileHelper);
    }
    ...
    protected void addTags() {
        for (BlockMapping<?> block : SFBlocks.BLOCKS) {
            if (!block.getTags().isEmpty()) {
                for (TagKey<Block> tag : block.getTags()) {
                    tag(tag).add(block.getRegisteredBlock().get());
                }
            }
        }
    }
